...
BATOCERA_APPS_VERSION = 1.1
BATOCERA_APPS_LICENSE = Various
BATOCERA_APPS_STRIP = NO
BATOCERA_APPS_TOOLCHAIN = manual
BATOCERA_APPS_DEPENDENCIES = host-zstd

BATOCERA_APPS_SITE = https://github.com/shy1132/VacuumTube/releases/download/v1.8.1
BATOCERA_APPS_SOURCE = VacuumTube-x86_64.AppImage
BATOCERA_APPS_CHROME_TAG = continuous
BATOCERA_APPS_CHROME_SOURCE = Google-Chrome-stable-150.0.7871.114-1-x86_64.AppImage
BATOCERA_APPS_FIREFOX_TAG = continuous-stable
BATOCERA_APPS_FIREFOX_SOURCE = Firefox-stable-152.0.5-x86_64.AppImage
...
BATOCERA_APPS_PROTONUPQT_SOURCE = ProtonUp-Qt-2.15.1-x86_64.AppImage
BATOCERA_APPS_STEAM_ROM_MANAGER_SOURCE = Steam-ROM-Manager-2.5.43.AppImage
BATOCERA_APPS_EXTRA_DOWNLOADS = \
	...
	https://github.com/DavidoTek/ProtonUp-Qt/releases/download/v2.15.1/$(BATOCERA_APPS_PROTONUPQT_SOURCE) \
	https://github.com/SteamGridDB/steam-rom-manager/releases/download/v2.5.43/$(BATOCERA_APPS_STEAM_ROM_MANAGER_SOURCE) \
	https://github.com/unknownskl/greenlight/releases/download/v2.4.2/Greenlight-2.4.2.AppImage \
	...
	https://github.com/peazip/PeaZip/releases/download/11.2.0/peazip_portable-11.2.0.LINUX.Qt6.x86_64.tar.gz
...
define BATOCERA_APPS_EXTRACT_CMDS
	...
	fetch_appimage "$(DL_DIR)/$(BATOCERA_APPS_DL_SUBDIR)/VacuumTube-x86_64.AppImage" \
		"$(BATOCERA_APPS_SITE)/$(BATOCERA_APPS_SOURCE)" \
	...
	fetch_appimage "$(DL_DIR)/$(BATOCERA_APPS_DL_SUBDIR)/Greenlight-2.4.2.AppImage" \
		"https://github.com/unknownskl/greenlight/releases/download/v2.4.2/Greenlight-2.4.2.AppImage" \
	...
	fetch_appimage "$(DL_DIR)/$(BATOCERA_APPS_DL_SUBDIR)/chiaki-ng.AppImage_x86_64" \
		"https://github.com/streetpea/chiaki-ng/releases/download/v1.10.0/chiaki-ng.AppImage_x86_64" \
		"$(@D)/chiaki.AppImage"; \
	if [ -f "$(DL_DIR)/$(BATOCERA_APPS_DL_SUBDIR)/peazip_portable-11.2.0.LINUX.Qt6.x86_64.tar.gz" ]; then \
		cp "$(DL_DIR)/$(BATOCERA_APPS_DL_SUBDIR)/peazip_portable-11.2.0.LINUX.Qt6.x86_64.tar.gz" "$(@D)/peazip-qt6.tar.gz"; \
	else \
		wget -O "$(@D)/peazip-qt6.tar.gz" "https://github.com/peazip/PeaZip/releases/download/11.2.0/peazip_portable-11.2.0.LINUX.Qt6.x86_64.tar.gz"; \
	fi; \
...
define BATOCERA_APPS_INSTALL_TARGET_CMDS
	...
	if ar t $(@D)/parsec.deb | grep -q '^data.tar.xz$$'; then \
		...
		ar p $(@D)/parsec.deb data.tar.zst | $(HOST_DIR)/bin/zstd -dc | tar -x -C $(TARGET_DIR); \
	else \
